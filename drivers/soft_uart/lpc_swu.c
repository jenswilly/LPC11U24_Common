 ...
 * P0.17:  SWU_RX  [CAP0.0]- software UART Rx pin
 *
 *********************************************************/
...
static void swu_setup_tx(void) {
	...
	if (tx_fifo_wr_ind != tx_fifo_rd_ind) //data to send, proceed
	{
		...
		reference = LPC_CT32B0->TC + BIT_LENGTH; //get the reference from TIMER0
		for (i = 0; i != cnt_edges; i++) //recalculate toggle points...
			edge[i] = (edge[i] + reference) //... an adjust for the...
					& 0x3FFFFFFF; //... timer range
		LPC_CT32B0->MR3 = edge[0]; //load MR3
		LPC_CT32B0->MCR = LPC_CT32B0->MCR | (1 << 9); //enable interrupt on MR3 match
		LPC_CT32B0->EMR = LPC_CT32B0->EMR | (3 << 10); //enable toggle on MR3 match
	}
 ...
 **
 ******************************************************************************/
void swu_isr_tx(LPC_CTxxBx_Type* const TX_ISR_TIMER) {
 ...
 **
 ******************************************************************************/
void swu_isr_rx(LPC_CTxxBx_Type* const RX_ISR_TIMER) {
 ...
 **
 ******************************************************************************/
void swu_init(LPC_CTxxBx_Type* const UART_TIMER) {
	...
	UART_TIMER->TCR = 0x01; //let TIMER0_32 running
	NVIC_EnableIRQ(TIMER_32_0_IRQn); //Enable the TIMER0_32 Interrupt

	while (0 == (LPC_GPIO->PIN[0] & (1 << PIN_SW_RX)))
